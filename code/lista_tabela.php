...
// Se estiver fazendo uma requisição AJAX (GET) para buscar dados
if (isset($_GET['tabela'])) {
    header('Content-Type: application/json');
    echo json_encode(listarDados($_GET['tabela']));
    exit;
}
?>
...
<body>
    ...
    <select id="selectTabelas">
        ...
        <option value="aula_agendada">aula_agendada</option>
        <option value="avaliacao_fisica">avaliacao_fisica</option>
        <option value="cargo">cargo</option>
        <option value="categoria_produto">categoria_produto</option>
        <option value="cupom_desconto">cupom_desconto</option>
        <option value="dieta">dieta</option>
        <option value="dieta_alimentar">dieta_alimentar</option>
        <option value="endereco">endereco</option>
        <option value="exercicio">exercicio</option>
        <option value="forum">forum</option>
        <option value="funcionario">funcionario</option>
        <option value="historico_treino">historico_treino</option>
        ...
        <option value="resposta_forum">resposta_forum</option>
        <option value="treino">treino</option>
        <option value="treino_exercicio">treino_exercicio</option>
    ...
    <script>
    document.getElementById('selectTabelas').addEventListener('change', function() {
        ...
        `;
            ...
            fetch('?tabela=' + tabelaSelecionada)
                .then(response => {
                    // Verifica se a resposta foi bem-sucedida (status 200)
                    if (!response.ok) {
                        throw new Error('Erro na requisição: ' + response.statusText);
                    }
                    return response.text(); // Obtemos o conteúdo como texto
                })
                .then(texto => {
                    // Verifica se a resposta é JSON
                    try {
                        var dados = JSON.parse(texto); // Tenta fazer o parse do texto como JSON
                        setTimeout(() => { // Delay de 4 segundos
                            resultado.innerHTML = ''; // Limpa o spinner de carregamento
                        ...
                        resultado.innerHTML = 'Erro ao buscar dados: resposta não é um JSON válido.';
                    }
                })
                .catch(error => {
                    console.error('Erro:', error);
                    resultado.innerHTML = 'Erro ao buscar dados: ' + error.message;
                });
        } else {
...
<!-- 
    <script>
        ...
        select.addEventListener("change", async () => {
            ...
            if (!tabela) {
                resultado.innerHTML = "";
                resultado.classList.remove("resultado-loading");
                return;
            }

            try {
                // Adiciona a classe temporária
                resultado.classList.add("resultado-loading");
                resultado.innerHTML = `
        <div class="loader">
            <span class="l">
            <svg
        ...
        `;
                ...
                // Aguarda 5 segundos simulando carregamento
                ...
                resultado.innerHTML = `<p style="color: red;">Erro ao buscar os dados: ${err.message}</p>`;
            } finally {
                // Remove a classe depois que tudo for carregado (mesmo se der erro)
                resultado.classList.remove("resultado-loading");
            }
        });
    </script> -->
