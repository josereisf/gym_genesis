...
function removerItem(botao) {
  let id = $(botao).val();
  let card = $(botao).closest(".flex.items-center"); // O bloco inteiro do produto
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      card.fadeOut(300, function () {
        $(botao).remove();
        atualizar_total(); // ✅ Agora chama a função correta depois de remover
      });
...
function atualizar_total() {
  ...
  $(".subtotal_compra, #totalCompra").text("R$ " + total.toFixed(2).replace(".", ","));
}
